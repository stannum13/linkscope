...
from photon_link_lab.symbols import bits_per_symbol, symbol_indices_to_gray_bits
...
def estimate_ber(tx_indices: np.ndarray, rx_indices: np.ndarray, order: int) -> tuple[float, float]:
    ...
    symbol_errors = np.count_nonzero(tx != rx)
    ser = symbol_errors / n
    bit_errors = _count_gray_bit_errors(tx, rx, order)
    ber = bit_errors / (n * bits_per_symbol(order))
...
def ber_confidence_metrics(
    ...
    The simulator makes hard symbol decisions. SER is counted over symbols,
    while BER is counted over Gray-coded PAM symbol bits. The historical
    ``ber_proxy`` field preserves the old ``SER / bits_per_symbol`` estimate
    for compatibility, but FEC comparisons use the bit-counted BER bound.
    """
    ...
    n = min(len(tx_indices), len(rx_indices))
    symbol_bits = bits_per_symbol(order)
    ...
    if n == 0:
        return {
            ...
            "symbol_errors": 0.0,
            "bit_errors": 0.0,
            "bits_per_symbol": float(symbol_bits),
            "ser": 0.0,
            "ber": 0.0,
            "ber_proxy": 0.0,
            f"ser_upper_{suffix}": 1.0,
            f"ber_upper_{suffix}": 1.0,
    ...
    bit_errors = _count_gray_bit_errors(tx, rx, order)
    bit_count = n * symbol_bits
    ser = symbol_errors / n
    ber_proxy = ser / symbol_bits
    ber = bit_errors / bit_count
    ser_upper = _wilson_upper_bound(symbol_errors, n, confidence)
    ber_upper = _wilson_upper_bound(bit_errors, bit_count, confidence)
...
def _count_gray_bit_errors(tx_indices: np.ndarray, rx_indices: np.ndarray, order: int) -> int:
    tx_bits = symbol_indices_to_gray_bits(np.asarray(tx_indices, dtype=int), order)
    rx_bits = symbol_indices_to_gray_bits(np.asarray(rx_indices, dtype=int), order)
