...
def dispersion_ui_penalty(config: WDMConfig, symbol_rate_gbaud: float) -> float:
    """First-order UI spread estimate from fiber dispersion."""
...
def simulate_wdm(
    ...
    dispersion_ui = dispersion_ui_penalty(wdm, cfg.symbol_rate_gbaud)
    rows = []
    nominal_detuning_nm = mod.resonance_wavelength_nm - cfg.wavelength_nm
    for channel, wavelength in enumerate(wavelengths):
        channel_cfg = cfg.with_updates(wavelength_nm=float(wavelength), seed=cfg.seed + channel)
        ...
        penalty = 1.0 + 10.0 * crosstalk_ratio + dispersion_ui
        rows.append(
            {
                ...
                "crosstalk_penalty_db": linear_to_db(1.0 + crosstalk_ratio),
                "dispersion_ui": dispersion_ui,
            }
        )
    return rows
